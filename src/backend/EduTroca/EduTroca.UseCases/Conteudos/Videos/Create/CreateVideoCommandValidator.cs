...
{
    private const long MaxVideoSize = 2L * 1024 * 1024 * 1024;
    ...
    public CreateVideoCommandValidator()
    {
        ...
        RuleFor(x => x.video)
            ...
            .NotNull().WithMessage("O arquivo de vídeo é obrigatório.")
            .Must(p => p.ContentType.StartsWith("video/"))
                .WithMessage("O arquivo deve ser um vídeo válido.")
            .Must(p => p.Length > 0 && p.Length <= MaxVideoSize)
                .WithMessage("O vídeo excede o limite de 2GB.");
